...
What this does **not** do: it is not proof of wallet ownership. A party who requests
their own 402 for the same resource ahead of time holds a token of their own and can
still race you with it (the issue's own caveat for this direction). Submit your hash
promptly after it confirms. The EIP-3009 (`PAYMENT-SIGNATURE`) path and the Solana
partially-signed-transaction path are not exposed to this at all and need no token.
...
tier for a call that would deliver nothing. If the probe cannot run (cache
unconfigured or unreachable) the cached price is quoted as before; a miss on the paid
call is still not charged, because settlement is gated on the route's 2xx response.
