...
| `duplicate_settlement` | `Nonce already used (replay detected)`: the nonce was submitted within the last five minutes, or a settled-transfer proof (EVM `txHash`, confirmed Solana `txSignature`) was already redeemed within its 24-hour window. | Use a fresh random nonce per request. A settled transfer pays exactly once. |
| `expired_payment` | `validBefore` has passed. Authorizations are valid for five minutes. | Sign a fresh authorization; never reuse a signed payload. |
...
| Growth | TBD | 50,000 |
| Pro | 1,000 | 250,000 |
| x402 (no key) | No limit | Unlimited (pay-per-use) |
