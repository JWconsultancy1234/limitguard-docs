...
| **Indie** | TBD | 1,000 / mo | `lg_live_` API key — upgraded via x402 ($29) |
| **Starter** | 300 / min | 10,000 / mo | `lg_live_` API key — upgraded via x402 ($99) |
| **Growth** | TBD | 50,000 / mo | `lg_live_` API key — upgraded via x402 ($299) |
| **Pro** | 1,000 / min | 250,000 / mo | `lg_live_` API key — upgraded via x402 ($999) |
...
<CardGroup cols={2}>
  <Card title="Upgrade Your Plan" icon="arrow-up" href="/authentication">
    Move from Free to Indie, Starter, Growth, or Pro for higher per-minute and monthly limits.
  </Card>
...
Available tiers: `free`, `sandbox`, `indie`, `starter`, `growth`, `pro`. See [Authentication](/authentication#api-key-authentication) for quotas and upgrade prices.
