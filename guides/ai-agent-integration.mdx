...
```json Response
{
  "name": "limitguard",
  "version": "1.0",
  "description": "LimitGuard Trust Intelligence - entity verification and risk scoring tools for AI agents",
  "tools": [
    {
      "name": "check_entity",
      "description": "MCP tool: full entity trust check for LLM agents. Same as /v1/entity/check with MCP-native interface.",
      "inputSchema": {
        "type": "object",
        "properties": {
          "entity_name": { "type": "string" },
          "country": { "type": "string", "description": "ISO country code (NL, BE, etc.)" },
    ...
    {
      "name": "check_agent",
      "description": "MCP tool: verify AI agent identity and reputation score before inter-agent transactions.",
      "inputSchema": {
        "type": "object",
        "properties": {
    ...
    {
      "name": "trust_score",
      "description": "MCP tool: get entity trust score (0-100) with cluster assignment and recommendation.",
      "inputSchema": {
        "type": "object",
    ...
    {
      "name": "verify_wallet",
      "description": "MCP tool: verify blockchain wallet address. Checks on-chain activity and risk flags.",
      "inputSchema": {
        "type": "object",
        "properties": {
          "wallet_address": { "type": "string" },
          "chain_id": { "type": "string", "description": "CAIP-2 chain ID (default: eip155:8453)" }
        },
...
All 5 tools are priced via x402 — see [Pricing](/pricing#mcp-tool-endpoints). They are not free just because they're callable as native tools.
...
Truncated to three entries; the live response lists all 18 priced endpoints. `price_raw` is the default (`fresh`) price in USDC 6-decimal units. The listing does **not** break out per-quality-tier prices — for `cached` or `enhanced` amounts, see [Pricing](/pricing#tiered-pricing) or read the `amount` from the 402 response for the quality you requested.
...
def verify_entity_x402(
    ...
    tier_prices = {"cached": 100_000, "fresh": 850_000, "enhanced": 1_500_000}
...
async function verifyEntityX402(entityName, country, signer, quality = "fresh") {
  const tierPrices = { cached: 100_000, fresh: 850_000, enhanced: 1_500_000 };
...
| Enhanced | `enhanced` | $1.50 (1,500,000) | $1.00 (1,000,000) | $3.00 (3,000,000) | Live + premium sources |
...
class TrustGatedAgent:
    ...
    TIER_PRICES = {
        "/v1/entity/check": {"cached": 100_000, "fresh": 850_000, "enhanced": 1_500_000},
        "/v1/risk/score": {"cached": 100_000, "fresh": 650_000, "enhanced": 1_000_000},
        "/v1/reputation/score": {"cached": 100_000, "fresh": 650_000, "enhanced": 1_000_000},
        "/v1/kyb/check": {"cached": 250_000, "fresh": 1_500_000, "enhanced": 3_000_000},
    }
