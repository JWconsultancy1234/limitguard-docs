...
API key users bypass x402 payment on API calls — each call draws down your tier's monthly quota instead. Rate limits and monthly quotas apply based on tier; quotas reset on the first day of each calendar month (UTC).
...
`POST /v1/keys/create` can only provision `free` or `sandbox` tiers directly. To move up the ladder, create a free key first, then pay via `GET /v1/keys/upgrade/{tier}` (x402 required even if you already hold a key — see [Pricing](/pricing#api-key-tier-upgrades)):

| Tier | Monthly quota | Upgrade price |
|------|---------------|---------------|
| `free` / `sandbox` | 500 calls (a ceiling, not an allowance: REST calls are still paid per call over x402) | — (self-service) |
| `indie` | 1,000 calls | $29 |
| `starter` | 10,000 calls | $99 |
| `growth` | 50,000 calls | $299 |
| `pro` | 250,000 calls | $999 |

Check remaining quota any time with `GET /v1/keys/usage`.
