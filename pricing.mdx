...
All prices are in USDC. Pay per API call via the [x402 protocol](/x402-protocol), or hold an [API key](/authentication) and draw down a monthly call quota instead.
...
<CardGroup cols={2}>
  ...
  <Card title="API Key Subscription" icon="key">
    API key holders bypass x402 on regular API calls, drawing down a monthly quota instead. Tier upgrades themselves are purchased via x402.
