...
| `GET /v1/compliance/report/{id}` | $0.15 | $0.50 | $2.00 |
...
The [5 MCP tools](/guides/ai-agent-integration#mcp-model-context-protocol) are priced the same as their REST equivalents — they are **not** free, even though they're callable as native LLM tools:

| Endpoint | Tool | Price |
|----------|------|-------|
| `POST /v1/mcp/check-entity` | `check_entity` | $0.85 |
| `POST /v1/mcp/risk-score` | `risk_score` | $0.65 |
| `POST /v1/mcp/check-agent` | `check_agent` | $0.10 |
| `POST /v1/mcp/trust-score` | `trust_score` | $0.10 |
| `GET /v1/mcp/verify-wallet` | `verify_wallet` | $0.10 |

## API Key Tier Upgrades

Pay once via x402 to unlock a higher monthly call quota on an existing API key. This is the only case where x402 payment is required even if you already hold an API key — see [Authentication](/authentication#api-key-authentication) for the full tier ladder.

| Endpoint | Price | Unlocks |
|----------|-------|---------|
| `GET /v1/keys/upgrade/indie` | $29 | 1,000 calls/mo |
| `GET /v1/keys/upgrade/starter` | $99 | 10,000 calls/mo |
| `GET /v1/keys/upgrade/growth` | $299 | 50,000 calls/mo |
| `GET /v1/keys/upgrade/pro` | $999 | 250,000 calls/mo |
